...
export function TwistingRibbon({
  ...
  useEffect(() => {
    ...
    function buildEdges(pts: { x: number; y: number }[], normals: { nx: number; ny: number }[], time: number) {
    ...
    function drawShadow(tops: { x: number; y: number }[], bots: { x: number; y: number }[], isDark: boolean) {
    ...
    function drawRibbon(tops: { x: number; y: number }[], bots: { x: number; y: number }[], twists: number[], time: number, isDark: boolean) {
      ...
      for (let i = 0; i < segments; i++) {
        ...
        if (Math.abs(twists[i]) > EDGE_MIN_TWIST) {
          ...
          ctx!.beginPath(); ctx!.moveTo(tops[i].x, tops[i].y); ctx!.lineTo(tops[i+1].x, tops[i+1].y); ctx!.stroke();
          ctx!.beginPath(); ctx!.moveTo(bots[i].x, bots[i].y); ctx!.lineTo(bots[i+1].x, bots[i+1].y); ctx!.stroke();
        }
      }
    }
