...
function HSegment({
  ...
  color: string; layers: number; staggerDelay: number; hovered: boolean; dimmed: boolean;
  renderPattern?: (id: string, color: string) => ReactNode;
          ...
          {rings.map((r, i) => {
            ...
            return (
              <HRing
                ...
                color={color} d={r.d} fill={ringFill}
                hovered={hovered} opacity={r.opacity} ringIndex={i} totalRings={layers}
              />
            );
          })}
        </svg>
...
function SegmentLabel({
  ...
  formatPercentage, formatValue, index, staggerDelay,
  layout = "spread", orientation, align = "center",
}: {
  ...
  layout?: "spread" | "grouped"; orientation?: "vertical" | "horizontal";
  ...
  const labelEl = showLabels && (
    <span className="whitespace-nowrap font-medium text-muted-foreground text-xs">{stage.label}</span>
  );
        ...
        <>
            ...
            {data.map((stage, i) => {
              ...
              return horiz ? (
                <HSegment
                  ...
                  index={i} layers={layers} normEnd={normEnd} normStart={normStart}
            ...
            >
              {Array.from({ length: n - 1 }, (_, i) => {
                ...
                if (horiz) {
                  ...
                  return <line key={`grid-${idx}`} stroke={gridLineColor} strokeOpacity={gridLineOpacity} strokeWidth={gridLineWidth} x1={x} x2={x} y1={0} y2={H} />;
                }
          ...
          {data.map((stage, i) => {
            ...
            return (
              ...
              >
                <SegmentLabel
                  align={labelAlign} formatPercentage={formatPercentage} formatValue={formatValue}
                  index={i} isHorizontal={horiz} layout={labelLayout} orientation={labelOrientation}
                  pct={pct} showLabels={showLabels} showPercentage={showPercentage}
